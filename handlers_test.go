...
func TestExternalCoordinatorServer(t *testing.T) {
	...
	t.Run("mergePairData", func(t *testing.T) {
		...
		t.Run("Adjust Success Range", func(t *testing.T) {
			...
			expectedSuccessAmtSat := newData.FailAmtSat - 1
			...
			if existingData.SuccessAmtSat != expectedSuccessAmtSat {
				t.Errorf("Expected SuccessAmtSat %v, got %v", expectedSuccessAmtSat, existingData.SuccessAmtSat)
			}

			// Checking if SuccessAmtMsat is adjusted correctly.
			if existingData.SuccessAmtMsat !=
				expectedSuccessAmtMsat {
				t.Errorf("Expected SuccessAmtMsat %v, got %v", expectedSuccessAmtMsat, existingData.SuccessAmtMsat)
			}
		})
		...
		t.Run("Adjust Failure Range", func(t *testing.T) {
			...
			// Expected values after merge.
			expectedFailAmtSat := newData.SuccessAmtSat + 1
			...
			if existingData.FailAmtSat != expectedFailAmtSat {
				t.Errorf("Expected FailAmtSat %v, got %v", expectedFailAmtSat, existingData.FailAmtSat)
			}

			// Checking if FailAmtMsat is adjusted correctly.
			if existingData.FailAmtMsat != expectedFailAmtMsat {
				t.Errorf("Expected FailAmtMsat %v, got %v", expectedFailAmtMsat, existingData.FailAmtMsat)
			}
		})
